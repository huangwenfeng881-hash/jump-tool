        ...
        for (seg in descSegs) {
            ...
            while (j < n - 1) {
                ...
                if (deltas.size >= 3) {
                    ...
                    if ((dv <= med * 0.6 || dv <= 0.007) && j + 4 < n && rawF[j + 2] != null && rawF[j + 3] != null && rawF[j + 4] != null) {
                        val d2 = rawF[j + 2]!! - f1
            ...
            var lo = -1
            // 起跳检测：阈值 0.6/0.4 → 0.4/0.25（更早触发，把稳定值 50 帧提前到 48 帧，与 JS 一致）
            for (q in m0 + 1 until n - 1) {
                val v = sH[q]
                if (v == null) continue
                if (v > mVal + 0.4 && sH[q + 1] != null && sH[q + 1]!! >= mVal + 0.25) { lo = q; break }
            }
