// 易支付(ezfp.cn) 公共库：MD5 签名 / 验签 / 下单 / 查询
const EZFP_API = 'https://www.ezfp.cn/mapi.php';
const EZFP_QUERY = 'https://www.ezfp.cn/api.php';

// MD5（blueimp 参考实现，逐轮展开已验证）
...
// 易支付签名：参数按 ASCII 排序拼接 a=b&c=d，sign/sign_type/空值不参与，再拼接 KEY，md5 小写
...
export function verify(params, key) {
  const expect = sign(params, key);
  const got = String(params.sign || '').toLowerCase();
  return expect === got;
}

// 服务端下单（mapi.php），返回 {code,msg,trade_no,payurl,qrcode,urlscheme}
export async function createPayment(params, key) {
  const body = new URLSearchParams();
  Object.keys(params).forEach((k) => {
    if (params[k] !== '' && params[k] != null) body.append(k, params[k]);
  });
  body.append('sign', sign(params, key));
  body.append('sign_type', 'MD5');
  const res = await fetch(EZFP_API, {
    method: 'POST',
